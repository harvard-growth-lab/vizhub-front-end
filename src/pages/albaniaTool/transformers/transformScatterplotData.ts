import { Factors, NACEIndustry, RCADirection } from "../graphql/graphQLTypes";
...
const transformScatterplotData = (
  rawFactors: Factors[],
  ...
  rawFactors.forEach((rawDatum) => {
    if (rawDatum) {
      const { naceId, avgViability, avgAttractiveness, rca } = rawDatum;
      if (
        naceId !== null &&
        avgViability !== null &&
        avgAttractiveness !== null &&
        rca !== null
      ) {
        const targetNaceIndustry = rawNaceData.find(
          (node) => node && node.naceId === naceId,
        );
        if (
          targetNaceIndustry &&
          targetNaceIndustry.name &&
          targetNaceIndustry.code
        ) {
          const parentTarget = rawNaceData.find(
            (datum) =>
              targetNaceIndustry.parentId !== null &&
              datum.naceId === targetNaceIndustry.parentId.toString(),
          );
          ...
          if (parentTarget && parentTarget.parentId !== null) {
            grandparentTarget = rawNaceData.find(
              (datum) =>
                ...
                datum.naceId === parentTarget.parentId.toString(),
            );
          } else {
          ...
          csvData.push({
            ...
            "2-Digit Industry Name":
              parentTarget && parentTarget.name !== null
                ? parentTarget.name
                : "",
            "2-Digit Industry NACE Code":
              parentTarget && parentTarget.code !== null
                ? parentTarget.code
                : "",
            "Sector Name":
              grandparentTarget && grandparentTarget.name !== null
                ? grandparentTarget.name
                : "",
            "Sector NACE Code":
              grandparentTarget && grandparentTarget.code !== null
          ...
          scatterPlotData.push({
            ...
            `,
            fill:
              rca === RCADirection.LessThanOne
                ? colorScheme.dataSecondary
                : colorScheme.data,
            naceId,
          });
        }
...
export const updateScatterPlotData = (
  ...
  return scatterPlotData.map((datum) => {
    ...
    const fill =
      selectedIndustry && selectedIndustry.label === datum.label
        ? rgba(existingFill, 0.4)
        : rgba(existingFill, 0.5);
    const highlighted =
      selectedIndustry && selectedIndustry.label === datum.label ? true : false;
    const onClick = () =>
      setSelectedIndustry({ value: datum.naceId, label: datum.label });
    return { ...datum, fill, highlighted, onClick };
  });
};

export default transformScatterplotData;
