...
import { Factors, NACEIndustry } from "../graphql/graphQLTypes";
...
const AttractivenessRadarChart = (props: Props) => {
  const { industryName, factors, naceId, rawNaceData } = props;
  if (factors) {
    let attractivenessData: RadarChartDatum[] = [];
    const attractivenessCsvData: any = {
      "3-Digit Industry Name": industryName,
    };
    if (factors.aWage !== null) {
      attractivenessData.push({
        label: "High Relative\nWages",
        value: factors.aWage,
      });
    ...
    if (factors.aYouth !== null) {
      attractivenessData.push({
        label: "High {{SPACE_OR_LINE}}Youth\nEmployment",
        value: factors.aYouth,
      });
    ...
    if (targetNaceIndustry) {
      if (targetNaceIndustry.code) {
        attractivenessCsvData["3-Digit Industry NACE Code"] =
...
export default AttractivenessRadarChart;
