...
const AttractivenessRadarChart = (props: Props) => {
  ...
  if (factors) {
    ...
    if (factors.aRelativeDemand !== null) {
      viabilityData.push({
        label: "Relative Demand in the Country and Region",
        value: parseFloat(factors.aRelativeDemand.toFixed(3)),
      });
      viabilityCsvData["Relative Demand in the Country and Region"] =
        factors.aRelativeDemand;
    }
    if (factors.aResiliency !== null) {
      viabilityData.push({
        label: "Resilience\nto Shocks",
        value: parseFloat(factors.aResiliency.toFixed(3)),
      });
    ...
    if (factors.aEmploymentGroupsInterest !== null) {
      viabilityData.push({
        label: "Likelihood of\nEmploying Groups\nof Interest",
        value: parseFloat(factors.aEmploymentGroupsInterest.toFixed(3)),
      });
      viabilityCsvData["Likelihood of Employing Groups of Interest"] =
        factors.aEmploymentGroupsInterest;
    }
    if (factors.aFdi !== null) {
      viabilityData.push({
        label: "Propensity to\nAttract FDI",
        value: parseFloat(factors.aFdi.toFixed(3)),
      });
    ...
    if (factors.aExportPropensity !== null) {
      viabilityData.push({
        label: "Export\nPropensity",
        value: parseFloat(factors.aExportPropensity.toFixed(3)),
      });
