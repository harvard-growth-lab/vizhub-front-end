...
import GradientHeaderPanelSearch, {
  IntroTextPosition,
} from '../../components/text/headers/GradientHeaderPanelSearch';
...
const NamibiaToolLayout = (props: Props) => {
  ...
  const [selectedIndustry, setSelectedIndustry] = useState<Datum>(
    initialSelectedIndustry as Datum,
  );
  const { productClass, id } = extractIdAndClass(selectedIndustry.id as string);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const updateSelectedIndustry = (val: Datum | null) => {
    if (val) {
  ...
  const links: NavItem[] = [
    ...
    },
    {
      label:
        'Nearby ' +
        (productClass === ProductClass.HS ? 'Products' : 'Industries'),
  ...
  if (loading === true) {
    ...
    if (productClass === ProductClass.HS) {
      ...
      content = <FullPageError message={'Invalid Classification'} />;
    }
    nav = (
      <StickySideNav
      ...
      <GradientHeaderPanelSearch
        ...
        links={[
          ...
          },
          {
            ...
            target:
              'https://growthlab.hks.harvard.edu/applied-research/namibia',
          },
        ]}
