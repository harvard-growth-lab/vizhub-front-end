  ...
  if (rawDatum !== undefined) {
    const {
      jordanIndustry: {
        ...
        keywords,
        factors,
        nationality: { edges: nationalityEdges },
        schooling: { edges: schoolingEdges },
        occupation: { edges: occupationEdges },
        mapLocation: { edges: mapLocationEdges },
        wageHistogram: { edges: wageHistogramEdges },
        overTime: { edges: overTimeHistogramEdges },
        text: { edges: textEdges },
        control: { edges: controlEdges },
      },
    } = rawDatum;
    ...
    ******/
    const text: Text =
      textEdges && textEdges[0] && textEdges[0].node
        ? textEdges[0].node
        : {
            occupation: null,
            demographic: null,
    ...
    ******/
    const { geoJsonWithValues, minValue, maxValue } =
      generateGeoJsonData(mapLocationEdges);
    ...
    ******/
    const { overTimeHistogramData, overTimeHistogramCsvData } =
      generateOverTimeHistogramData(overTimeHistogramEdges);

    /****
      Get Control data
    ****/
    const control: Control =
      controlEdges && controlEdges[0] && controlEdges[0].node
        ? {
    ...
    const { scatterPlotData, csvData: scatterPlotCsvData } =
      generateScatterPlotData({
        rawDatum: rawIndustryList,
        id,
        setSelectedIndustry,
      });
