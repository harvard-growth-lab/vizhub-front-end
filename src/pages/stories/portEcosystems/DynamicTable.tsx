...
const DynamicTable = (props: Props) => {
  ...
  const tableHeader = columns.map(({ label, align }, i) => {
    ...
    const style: React.CSSProperties = {
      ...
      borderBottom:
        hideGridLines || verticalGridLines
          ? `solid 2px ${lightBorderColor}`
          : undefined,
      borderRight,
      justifyContent: alignToFlexValue(align ? align : Align.Left),
    };
    return (
      <Cell style={style} key={label}>
        {label}
      </Cell>
    );
  });
  const rows = data.map((d, i) => {
    ...
    return Object.keys(d).map(function (key: keyof Datum, j) {
      ...
      const style: React.CSSProperties = {
        ...
        gridRow,
        backgroundColor:
          gridRow % 2 === 0 || !color ? undefined : rgba(color, 0.2),
        borderBottom: hideGridLines ? "none" : undefined,
        fontSize,
        gridColumn,
        borderTop:
          d.naics_chapter !== "" && gridRow !== 2
            ? `1px solid #000`
            : undefined,
        fontWeight:
          d.naics_chapter !== "" && gridColumn === 1 ? 700 : undefined,
        justifyContent: alignToFlexValue(
          columns[gridColumn - 1] && columns[gridColumn - 1].align
            ? (columns[gridColumn - 1].align as Align)
            : Align.Left,
        ),
      };
      ...
      if (gridColumn) {
        if (d[key] !== null) {
          return (
            <Cell
              style={style}
              key={(d[key] as string | number).toString() + i + key}
            >
              {d[key]}
            </Cell>
          );
          ...
          return (
            <Cell style={style} key={i + key.toString()}>
              <GhostContent style={{ minWidth }} />
            </Cell>
          );
        }
      } else {
  ...
  return (
    <Root>
      ...
      <BodyRowsContainer style={rootStyle}>{rows}</BodyRowsContainer>
