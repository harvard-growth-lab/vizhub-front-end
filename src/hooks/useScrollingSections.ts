...
const useScrollingSections = ({ refs }: Options) => {
  const [section, setSection] = useState<number | null>(null);

  useEffect(() => {
    const windowHeight = window.innerHeight;
    const handleScroll = () => {
      const sectionTops = refs.map((node) =>
        node && node.current
          ? node.current.getBoundingClientRect().top + window.scrollY
          : 0,
      );
...
export default useScrollingSections;
